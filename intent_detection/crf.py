...
#
# This is a reusable CRF module, Batch training has been implemented and the foreach operation has
# been used to improve the for loop unroll on the sequence in previous versions. Implemented with
# gluon, it can be encapsulated as Gluon's neural network layer and interface with other
# network layers.
#
...
# @author：kenjewu
# @date：2018/11/05
# @url: https://github.com/kenjewu/CRF - code originally taken from here
...
class CRF(gluon.nn.Block):
    ...
    def __init__(self, tag2idx, ctx=mx.cpu(), **kwargs):
        """Constructor

        Args:
            tag2idx (dict): a dictionary of sequence tags, such as:
            {"B": 0, "I": 1, "O": 2, START_TAG: 3, STOP_TAG: 4}
            ctx (context of mxnet).
        """
    ...
    def _score_sentence(self, feats, tags):
        ...
        Args:
            ...
            tags (NDArray): An index representing the label of each symbol in a batch,
        ...
        # A matrix that retrieves the start tag of a batch of sentence symbol sequences.
        # shape：(batch_size, 1)
        ...
        score = states[2]
        score = score + self.transitions.data()[
            self.tag2idx[STOP_TAG], tags[:, int(tags.shape[1] - 1)]]
        return score
    ...
    def neg_log_likelihood(self, feats, tags):
        ...
        Args:
            feats (NDArray): a representative representation of each symbol representing the entire
                             sequence in a batch, shape is (seq_len, batch_size, tagset_size)
            tags (NDArray): An index representing the label of each symbol in a batch, shape is
                            (batch_size, seq_len)
