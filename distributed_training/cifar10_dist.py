...
train_data = gluon.data.DataLoader(cifar10_data,
                                      batch_size,
                                      sampler=SplitSampler(len(cifar10_data), store.num_workers, store.rank))
...
def train_batch(data, label, ctx, net, trainer):
    ...
    # Split and load data and label into multiple GPUs
    ...
    this_batch_size = data[0].shape[0]
...
net.save_parameters("trained-resnet.params")
