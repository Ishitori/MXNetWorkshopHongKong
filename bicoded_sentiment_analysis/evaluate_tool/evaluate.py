...
def eval_one_emotion(golds, results, emotion):
    ...
    p = tp / pp if pp > 0 else None
    r = tp / np if np > 0 else None
    f1 = (2 * p * r / (p + r)) if p is not None and r is not None else None

    print('[%s]\tP:%s\tR:%s\tF1:%s' % (emotion, p, r, f1))
...
def main():
    if len(argv) == 2:
        ...
        if len(golds) != len(results):
            ...
            print('USAGE: evaluate.py [gold_path] [result_path]')
            return
